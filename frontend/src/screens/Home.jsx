    ...
    return (
        <Fragment>
            ...
            <div className="mx-auto px-4 md:px-10 lg:px-20 xl:px-48 mt-4">
                <h1 className="text-3xl text-grey-400">Đi đến?</h1>
                <div className="flex flex-col md:flex-row gap-4 mt-4 mb-6">
                    ...
                    <Modal disableAutoFocus={true} open={isSearchOpen} onClose={() => setIsSearchOpen(false)} className="flex justify-center items-center">
                        <div className="w-full md:w-2/3 h-full md:h-2/3 bg-white md:rounded-lg">
                            ...
                            <input onKeyUp={(e) => e.key === "Enter" && searchHandler()} value={keyword} onChange={(e) => setKeyword(e.target.value)} type="text" placeholder="Đi đến" className=" outline-none bg-transparent w-full border-b border-gray-400 border-solid py-2 px-4 mt-4" />
                            <div className="mt-12 flex flex-col items-center">
                                ...
                                <p className="mt-5 text-gray-600">Tìm kiếm theo điểm đến</p>
                            </div>
                        </div>
                    </Modal>
                    ...
                    <Modal disableAutoFocus={true} open={isDateOpen} onClose={() => setIsDateOpen(false)} className="flex justify-center items-center mb-20">
                        <div className="flex flex-col bg-white pb-8 rounded-md">
                            ...
                            <div className="flex justify-center">
                                <Button variant="contained" className="w-72 h-12 " onClick={() => setIsDateOpen(false)}>Xong</Button>
                            </div>
                        </div>
                    </Modal>
                    ...
                    <Modal disableAutoFocus={true} open={isPersonOpen} onClose={() => setIsPersonOpen(false)} className="flex justify-center items-center">
                        <div className="w-full md:w-2/3 h-full md:h-2/3 bg-white md:rounded-lg">
                            ...
                            <h4 className="text-2xl text-gray-800 px-6 mt-6">Thêm khách</h4>
                            <div className="flex justify-between items-center my-5 px-8">
                                <h5 className=" text-xl text-gray-700 ">Người: </h5>
                            ...
                            <hr />
                            <div className="flex justify-between items-center my-5 px-8">
                                <h5 className="text-xl text-gray-700">Phòng: </h5>
                                <div className="flex justify-end items-center gap-2">
                            ...
                            <div className="flex justify-center mt-36">
                                <Button variant="contained" className="w-72 h-12 " onClick={travellersHandler}>Xong</Button>
                            </div>
                        </div>
                    </Modal>
                    <div className="flex justify-center mt-3 md:mt-0 items-center">
                        <button color="error" disabled={keyword.length < 1} variant="contained" className=" bg-red-500 rounded font-medium hover:bg-red-600 disabled:bg-red-400 w-72 !text-orange-50 md:w-24 lg:w-32 md:h-full h-12 text-inherit"
                            onClick={searchHandler}> Tìm kiếm</button>
                    </div>
                </div>
                <Fragment>
                    {hasSearched && (
                        <Fragment>
                            ...
                            <div>
                                <h2 className="text-xl text-center mb-4">Kết quả tìm kiếm</h2>
                                {hotels.length < 1 && <p className="text-center text-gray-600 my-48">Không tìm thấy khách sạn theo yêu cầu</p>}
                                {hotels.length > 0 && <p className="text-sm">{hotels.length} khách sạn được tìm thấy</p>}
                                {hotels?.map((hotel) => (
                    ...
                    {!hasSearched && (
                        <Fragment>
                            {isLoading ? (
                                ...
                                <div>
                                    <h2 className="text-xl font-medium mb-6 text-center">Địa điểm thịnh hành</h2>
                                    {hotels?.map((hotel) => (
                                        <HotelCard key={hotel._id} hotel={hotel} />
                                    ))}
                                </div>
