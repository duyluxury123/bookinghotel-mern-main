                    ...
                    <Fragment>
                        {!room ? <NotFound /> : (
                            <div className="w-[80%] sm:w-[60%] md:w-[70%] mx-auto mt-3">
                                <div className="flex flex-col md:flex-row gap-6 md:gap-4 items-center md:justify-between ">
                                    ...
                                    <div>
                                        <div className="flex gap-4">
                                            <h4 className="font-medium">Tên khách sạn:</h4><p className="font-normal text-blue-400"><Link to={`/hotel/${room?.hotel._id}`} >{room?.hotel.name}</Link></p>
                                        </div>
                                        <div className="flex gap-4">
                                ...
                                <h2 className="text-2xl font-medium text-center my-8">Cập nhật phòng</h2>
                                <form className="flex flex-col gap-4 px-2 max-w-fit mx-auto mb-8" onSubmit={(e) => handleSubmit(e)}>
                                    <div className="border border-solid border-gray-400 py-3 px-5 rounded">
                                        ...
                                        <input type="text" required={true} value={name} onChange={(e) => setName(e.target.value)} placeholder="Tên phòng" className="w-40 sm:w-60 md:w-80 ml-3 outline-none bg-transparent" />
                                    </div>
                                    <div className="border border-solid border-gray-400 py-3 px-5 rounded">
                                        <MeetingRoomIcon className="text-gray-600" />
                                        <input type="number" disabled={true} required={true} value={number} onChange={(e) => setNumber(e.target.value)} placeholder="Số phòng" className="w-40 sm:w-60 md:w-80 ml-3 outline-none bg-transparent" />
                                    </div>
                                    <div className="border border-solid border-gray-400 py-3 px-5 rounded">
                                        <RequestQuoteIcon className="text-gray-600" />
                                        <input type="number" required={true} value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Giá ngày" className="w-40 sm:w-60 md:w-80 ml-3 outline-none bg-transparent" />
                                    </div>
                                    <FormControl className="md:w-[25rem] w-60 sm:w-80">
                                        <InputLabel id="demo-single-checkbox-label" className="!text-gray-400">Loại phòng</InputLabel>
                                        <CustomSelect
                                    ...
                                    <Button variant="contained" type="submit" className="!bg-red-400 !py-4">Cập nhật</Button>
                                </form>
                            </div>
                        )}
                    </Fragment>
